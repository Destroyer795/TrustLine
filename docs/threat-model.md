...
| **Concurrent Draw Race Condition** | Agent sends 2 simultaneous ₹6,000 draws against ₹10,000 limit to double-spend. | PostgreSQL `select_for_update()` transaction locking agent and principal rows atomically. | Automated race script (`scripts/demo_concurrency.py`). |
